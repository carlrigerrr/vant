...
class cleanto_staff_commision{		
	...
	public function readall_booking(){
		$query = "select DISTINCT `service_id`,`client_id`,`staff_ids`,`order_id`, `booking_status`, `booking_date_time` from `ct_bookings` where `staff_ids`<>'' ";
		$result=mysqli_query($this->conn,$query);
		return $result;
	}
	public function get_staff_bookingandpayment_by_date($start, $end){
		$query = "select DISTINCT `service_id`,`client_id`,`staff_ids`,`order_id`, `booking_status`, `booking_date_time` from `ct_bookings` where `staff_ids`<>'' and `booking_date_time` between '".$start."' and '".$end."' ";
		$result=mysqli_query($this->conn,$query);
		return $result;
	}
	public function get_staff_bookingandpayment_by_dateser($start, $end, $sid){
		$query = "select DISTINCT `service_id`,`client_id`,`staff_ids`,`order_id`, `booking_status`, `booking_date_time` from `ct_bookings` where `staff_ids`<>'' and `service_id` = '".$sid."' and `booking_date_time` between '".$start."' and '".$end."' ";
		$result=mysqli_query($this->conn,$query);
		return $result;
	}
	...
	public function readall_ct_staff_commision_api(){
		$query = "SELECT * FROM `ct_staff_commission` ORDER BY `order_id` DESC LIMIT ".$this->limit." OFFSET ".$this->offset;
		$result = mysqli_query($this->conn,$query);
		return $result;
	}
	...
	public function staff_today_booking_details($staff_id){
		$today_date = date('Y-m-d');
		$arr = array();
		$query = "select DISTINCT `order_id`, `service_id`, `booking_date_time`, `staff_ids` from `ct_bookings`  where booking_date_time LIKE '%".$today_date."%' ORDER BY `order_id` DESC";
		
		$result=mysqli_query($this->conn,$query);
		if($result->num_rows > 0){
			while($value = mysqli_fetch_array($result)){
				$stafid = explode(',',$value['staff_ids']);
				if(sizeof((array)$stafid) > 0){
	...
	public function staff_future_booking_details($staff_id){
		$today_date = date('Y-m-d').' 23:23:59';
		$arr = array();
		$query = "select DISTINCT `order_id`, `service_id`, `booking_date_time`, `staff_ids` from `ct_bookings`  where booking_date_time > '".$today_date."' ORDER BY `order_id` DESC";
	...
	public function staff_past_booking_details($staff_id){
	...
	public function staff_service_details($staff_id){
		$arr = array();
		$query = "select DISTINCT `order_id`, `service_id`, `booking_date_time`, `staff_ids` from `ct_bookings`  ORDER BY `order_id` DESC";
