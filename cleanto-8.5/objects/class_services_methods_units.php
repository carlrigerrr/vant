...
class cleanto_services_methods_units{
		...
		public function add_services_method_unit(){
			$query="insert into `".$this->table_name."` (`id`,`services_id`,`methods_id`,`units_title`,`base_price`,`minlimit`,`maxlimit`,`status`,`position`,`limit_title`,`unit_symbol`,`half_section`,`uduration`)
 values(NULL,'".$this->services_id."','".$this->methods_id."','".$this->units_title."','".$this->base_price."','".$this->minlimit."','".$this->maxlimit."','".$this->status."','0','','','D','0')";
		...
		public function update_services_method_unit(){
			$query="update `".$this->table_name."` set  `units_title`='".$this->units_title."',`base_price`='".$this->base_price."',`minlimit`='".$this->minlimit."',`maxlimit`='".$this->maxlimit."',`limit_title`='".$this->maxlimit_title."',`unit_symbol`='".$this->unit_symbol."',`uduration`='".$this->duration."',`image`='".$this->image."', `predefine_image`='".$this->predefine_image."', `predefine_image_title` ='".$this->predefine_image_title."' where `id`=".$this->id;
			$result=mysqli_query($this->conn,$query);
			return $result;
        }
    ...
    public function get_maxlimit_by_service_methods_ids(){
        $query="select * from `".$this->table_name."` where `maxlimit` >0 and `services_id`=".$this->services_id." and `methods_id` = ".$this->methods_id." and `status` = 'E' ORDER BY `position`";
        $result=mysqli_query($this->conn,$query);
	...
	/*  function for read duration of unit */
	public function get_duration_value(){
		$query="SELECT `uduration` FROM `".$this->table_name."` WHERE `id`='".$this->units_id."'";
		$result=mysqli_query($this->conn,$query);
		$value=mysqli_fetch_assoc($result);
		return $value['uduration'];
	}
