  ...
  public function get_booking_details_appt($orderid)
{
    $query = "
        SELECT 
            ...
            `p`.`payment_method`,
            `oci`.`client_phone`,
            GROUP_CONCAT(DISTINCT `s`.`title` ORDER BY `s`.`title` SEPARATOR ', ') as `service_title`,
            `b`.`gc_event_id`,
            `b`.`gc_staff_event_id`,
            `b`.`staff_ids`,
            `b`.`reject_reason`,
            ...
            `b`.`client_id`,
            `p`.`recurrence_status`
        FROM `ct_bookings` as `b`
        INNER JOIN `ct_payments` as `p` ON `b`.`order_id` = `p`.`order_id`
        INNER JOIN `ct_order_client_info` as `oci` ON `b`.`order_id` = `oci`.`order_id`
        INNER JOIN `ct_services` as `s` ON `b`.`service_id` = `s`.`id`
    ...
    /* get all services of the bookings */
    public function get_services_ofbookings($orderid)
    {
        $query = "select `s`.`title` from `ct_services` as `s`,`ct_bookings` as `b` where `b`.`service_id` = `s`.`id` and `b`.`order_id` ='" . $orderid . "' group by `b`.`service_id`";
        $result = mysqli_query($this->conn, $query);
        return $result;
    }
    /* get all services methods of the bookings */
    public function get_services_methods_ofbookings($orderid)
    {
        $query = "select `sm`.`method_title` from `ct_services_method` as `sm`,`ct_bookings` as `b` where `b`.`method_id` = `sm`.`id` and `b`.`order_id` ='" . $orderid . "' group by `b`.`method_id`";
        $result = mysqli_query($this->conn, $query);
        return $result;
    }
    ...
    public function get_staff_detail_for_email($sid)
    {
    ...
    public function get_staff_ids_from_bookings($oid)
    {
    ...
    public function booked_staff_status()
    {
    ...
    /* Update GC Event ID */
    public function update_gc_event_id($last_id, $gc_event_id)
    {
    ...
    public function update_gc_staffid_event_id($last_id, $gc_event_id)
    {
    ...
    {
        $query = "SELECT `recurring_id`, `ct_bookings`.`order_id`, `staff_ids`, `gc_event_id`, `gc_staff_event_id` FROM `ct_bookings` INNER JOIN `ct_order_client_info` ON `ct_bookings`.`order_id` = `ct_order_client_info`.`order_id`";
    ...
    {
        $query = "select `title` from `" . $this->tablename1 . "` where `id`='" . $service_id . "'";
        $result = mysqli_query($this->conn, $query);
        $name = mysqli_fetch_array($result);
        return $name[0];
    }

    public function chk_and_get_booking()
    {
        $query = "select * from `" . $this->table_name . "` where `order_id`='" . $this->order_id . "'";
        $result = mysqli_query($this->conn, $query);
        $value = mysqli_fetch_array($result);
    ...
    public function delete_method_addons_booking()
    {
        $query = "delete from `" . $this->table_name . "` where `order_id`='" . $this->order_id . "'";
        $result = mysqli_query($this->conn, $query);
        if ($result) {
            $query1 = "delete from `" . $this->tablename5 . "` where `order_id`='" . $this->order_id . "'";
    ...
    public function edit_update_payments()
    {
        $query = "update `" . $this->tablename4 . "` set `amount`='" . $this->amount . "',`discount`='" . $this->discount . "',`taxes`='" . $this->taxes . "',`lastmodify`='" . $this->lastmodify . "',`net_amount`='" . $this->net_amount . "',`frequently_discount`='" . $this->frequently_discount . "',`frequently_discount_amount`='" . $this->frequently_discount_amount . "' where `order_id`='" . $this->order_id . "'";
        $result = mysqli_query($this->conn, $query);
        return $result;
    }

    public function edit_update_order_client()
    {
        $query = "update `" . $this->tablename2 . "` set `order_duration`='" . $this->order_duration . "' where `order_id`='" . $this->order_id . "'";
        $result = mysqli_query($this->conn, $query);
        return $result;
    }
}
