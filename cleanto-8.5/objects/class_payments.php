...
class cleanto_payments{
  ...
  */
  public function add_payments(){
    ...
    $query="insert into `".$this->table_name."` (`id`,`order_id`,`payment_method`,`transaction_id`,`amount`,`discount`,`taxes`,`partial_amount`,`payment_date`,`net_amount`,`lastmodify`,`frequently_discount`,`frequently_discount_amount`,`recurrence_status`,`tip`,`payment_status`) values(NULL,'".$this->order_id."','".$this->payment_method."','".$this->transaction_id."','".$this->amount."','".$discount."','".$this->taxes."','".$partial_amount."','".$this->payment_date."','".$this->net_amount."','".$this->lastmodify."','".$this->frequently_discount."','".$this->frequently_discount_amount."','".$this->recurrence_status."','".$this->tip_value."','".$this->payment_status."')";
	...
	public function get_stripe_reccurence_orders(){
		$query="select DISTINCT `order_id`,`transaction_id`,`payment_date` from `".$this->table_name."` where `payment_method`='Stripe-Reccurance' ";
		$result=mysqli_query($this->conn,$query);
		return $result;
	}
  ...
  public function get_whole_request(){
    $query="select * from `".$this->tablename_request."` where `staff_id`='".$this->staffid."'";
    $result=mysqli_query($this->conn,$query);
    return $result;
  }

  public function getallrequest(){
    $query = "SELECT * FROM `".$this->tablename_request."`";
    $result = mysqli_query($this->conn,$query);
    return $result;
  }
  
  public function update_request_status(){
    echo $query="update `".$this->tablename_request."` set `status`='".$this->status."' where `request_id`='".$this->reqid."' ";
    $result=mysqli_query($this->conn,$query);
  }
  
  public function staff_payment_request(){
  $query="insert into `".$this->tablename_request."` (`request_id`, `staff_id`,`email_id`, `request_amount`, `status`) values(NULL,'".$this->staffid."','".$this->email_id."','".$this->transfer_amount_value."','Pending')";
  $result=mysqli_query($this->conn,$query);
  return $result;
  }
}
