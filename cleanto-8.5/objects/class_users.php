...
/* 	public function edit_customer($user_id){
		echo $query  = "select DISTINCT `id` from `".$this->table_name."` where `id`='".$user_id."'";
		die;
		$result=mysqli_query($this->conn,$query);
		$val=mysqli_num_rows($result);
		return $val;
  ...
  public function get_client_info($orderid){
    ...
    $query="SELECT `cb`.`order_id`,`cb`.`order_date`,`cb`.`booking_date_time`,`cb`.`order_id`,`cs`.`title` as `service_name`,`cp`.`order_id`,`cp`.`payment_method`,`cp`.`transaction_id`,`cp`.`net_amount`,`cp`.`payment_status`,`cp`.`payment_method`,`cp`.`payment_date`,`cp`.`net_amount` FROM `ct_bookings`as `cb`,`ct_services` as `cs`,`ct_payments` as `cp` WHERE `cb`.`client_id` = '".$this->user_id."' AND `cb`.`service_id` = `cs`.`id` AND `cb`.`order_id` = `cp`.`order_id` limit ".$this->limit." offset ".$this->offset;
    $result = mysqli_query($this->conn,$query);
    return $result;
  }
	public function get_staff_payment_order_record(){
		$query="SELECT `cb`.`order_id`,`cb`.`order_date`,`cb`.`booking_date_time`,`cb`.`order_id`,`cs`.`title` as `service_name`,`cp`.`order_id`,`cp`.`payment_method`,`cp`.`transaction_id`,`cp`.`payment_status`,`cp`.`payment_method`,`cp`.`payment_date`,`cp`.`net_amount` FROM `ct_bookings`as `cb`,`ct_services` as `cs`,`ct_payments` as `cp` WHERE `cb`.`staff_ids` = '".$this->user_id."' AND `cb`.`service_id` = `cs`.`id` AND `cb`.`order_id` = `cp`.`order_id`";
		$result = mysqli_query($this->conn,$query);
		return $result;
  }
