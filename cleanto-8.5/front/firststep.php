...
include (dirname(dirname(__FILE__)) . '/objects/class_services_methods_units_rates.php');
...
$services_addon = new cleanto_services_addon();
$services_addon_rates = new cleanto_services_addon_rates();
$services_methods_units = new cleanto_services_methods_units();
$services_methods_units_rates = new cleanto_services_methods_units_rates();

$services_addon->conn = $conn;
$services_addon_rates->conn = $conn;
$services_methods_units->conn = $conn;
$services_methods_units_rates->conn = $conn;
...
if (isset($_POST['add_to_cart']))
	{
		/* $service_name = $booking->get_sername_byser_id($_POST['service_id']);
				echo "<pre>";print_r($service_name['title']);exit; */
	$json_array = array();
	...
	if ($_POST['type'] == 'method_units')
		...
		$services_methods_units->units_id = $_POST['units_id'];
		$duration = $services_methods_units->get_duration_value();
		if($duration == 0){
			$final_duration = 0;
		}else{
			if($_POST['s_m_qty'] == - 1){
				...
				$final_duration = ($duration*$_POST['s_m_qty']);
			}
		}
		
		}
	  elseif ($_POST['type'] == 'addon')
		{
		$method_name_without_space = 'ad_unit' . $_POST['units_id'];
		$services_addon->units_id = $_POST['units_id'];
		$duration = $services_addon->get_duration_value();
		if($duration == 0){
			$final_duration = 0;
		}else{
			if($_POST['s_m_qty'] == - 1){
	...
	if ($_POST['s_m_qty'] == -1)
        ...
        $hours = intval($final_duration_value/60);
        		...
        		$total_amount = $total_amount + (isset($_SESSION['tip_value']) ? $_SESSION['tip_value'] : 0);
		...
		if (isset($_SESSION['ct_cart']["method"]) && count($_SESSION['ct_cart']["method"]) == 0)
			...
			for ($i = 0; $i < (count($_SESSION['ct_cart']['method'])); $i++)
				...
				$_SESSION['ct_cart']["method"][$id]['duration'] = $final_duration;
				$json_array['service_id'] = $_POST['service_id'];
				$json_array['method_id'] = $_POST['method_id'];
				$json_array['units_id'] = $_POST['units_id'];
				$json_array['s_m_qty'] = $_POST['s_m_qty'];
				...
				$json_array['cart_tax'] = $general->ct_price_format($taxamount, $symbol_position, $decimal);
				$json_array['cart_sub_total'] = $general->ct_price_format($total, $symbol_position, $decimal); /* calculation end */
				$json_array['s_m_html'] = '<div class="ct-item ofh"><i data-service_id="' . $_POST['service_id'] . '" data-method_id="' . $_POST['method_id'] . '" data-units_id="' . $_POST['units_id'] . '"data-mnamee="' . $method_name_without_space . '" class="fa fa-times remove_item_from_cart cart_method_name"></i><span class="cart_method_name">' . $_POST['method_name'] . '</span> - <span class="cart_qty">' . $_POST['s_m_qty'] . '</span></div><div class="ct-price ofh cart_price">' . $general->ct_price_format_without_symbol($_POST['s_m_rate'], $decimal) . '</div>';
	...
	if ($_POST['coupon_code'] != ''){
		...
		if ($result){
			...
			if ($result['coupon_used'] < $result['coupon_limit'] && $curr_date <= $coupon_exp_date){
				...
				$jsonn_array['partial_amount'] = $general->ct_price_format_without_symbol($partial_amount, $decimal);
				$jsonn_array['remain_amount'] = $general->ct_price_format_without_symbol($remain_amount, $decimal);
				$jsonn_array['discount_status'] = "available";
				$jsonn_array['cart_discount'] = $general->ct_price_format_without_symbol($discount_values, $decimal);
				$jsonn_array['cart_tax'] = $general->ct_price_format_without_symbol($taxamount, $decimal);
				$jsonn_array['frequent_discount'] = '-' . $general->ct_price_format_without_symbol($_SESSION['freq_dis_amount'], $decimal);
				$jsonn_array['total_amount'] = $general->ct_price_format_total($alltot , $symbol_position, $decimal);
				$jsonn_array['cart_sub_total'] = $general->ct_price_format_without_symbol($totals, $decimal);
...
elseif (isset($_POST['coupon_reversed'])){
	...
	if ($result['coupon_used'] >= 0){
		...
		$jsonnn_array['cart_tax'] = $general->ct_price_format_without_symbol($taxamount, $decimal);
		$jsonnn_array['partial_amount'] = $general->ct_price_format_without_symbol($partial_amount, $decimal);
		$jsonnn_array['remain_amount'] = $general->ct_price_format_without_symbol($remain_amount, $decimal);
		$jsonnn_array['total_amount'] = $general->ct_price_format_total(($final_subtotal + $taxamount) , $symbol_position, $decimal);
		$jsonnn_array['frequent_discount'] = '-' . $general->ct_price_format_without_symbol($_SESSION['freq_dis_amount'], $decimal);
		$jsonnn_array['cart_sub_total'] = $general->ct_price_format_without_symbol($totals, $decimal);
...
elseif (isset($_POST['referral_check'])){
  if ($_POST['referral_code'] != ''){
    $referral_code = $_POST['referral_code'];
    $result = $coupon->check_referral_code($referral_code);
    $res_is = $result['id'];
    $reference_user_wallet_amt = $result['wallet_amount'];
    if ($result){
      if ($settings->get_option('ct_referral_type') == 'F'){
        $discount_values = $settings->get_option('ct_referral_value');
        $coupon->res_user_update_wallet_amt =  $reference_user_wallet_amt + $discount_values; 
        $reference_user_wallet = $coupon->update_refernce_user_wallet($res_is);
      } else {
        $reference_user_wallet = 0;
      }
    }
  }
}

elseif (isset($_POST['coupon_code_check'])){
  $jsonn_array = array();
  $_SESSION['ct_cart']['method'] = array_values($_SESSION['ct_cart']['method']);
  $c_rates = 0;
  for ($i = 0; $i < (count((array)$_SESSION['ct_cart']['method'])); $i++){
  ...
  if ($_POST['user_coupon_val'] != ''){
    ...
    if ($result){
      if ($result["coupon_used"]<$result["coupon_limit"]){
        if ($settings->get_option('ct_special_type') == 'F'){
          $discount_values = $settings->get_option('ct_special_offer_text');
        } else {
          $percent = $settings->get_option('ct_special_offer_text');
          $percentage_value = $percent / 100;
          $discount_values = $percentage_value * $allover_totals;
        }
...
/* Below code is use for reverse coupon */
elseif (isset($_POST['referral_reversed'])){
  $jsonnn_array = array();
  $frequently_discount->id = $_POST['frequently_discount_id'];
  $freq_dis_data = $frequently_discount->readone();
  $user_coupon_val = $_POST['display_user_coupon'];
  $result = $coupon->checkcouponcode($user_coupon_val);
  if ($result){
    $_SESSION['ct_cart']['method'] = array_values($_SESSION['ct_cart']['method']);
    $c_rates = 0;
...
elseif (isset($_POST['frequently_discount_set'])){
	/* below code for frequently_discount SET */
	$_SESSION['freq_dis_amount'] = $_POST["freq_discount_amount"];
}
...
elseif (isset($_POST['frequently_discount_check']))
	...
	$json_array['partial_amount'] = $general->ct_price_format_without_symbol($partial_amount, $decimal);
	$json_array['remain_amount'] = $general->ct_price_format_without_symbol($remain_amount, $decimal);
	$json_array['cart_tax'] = $general->ct_price_format_without_symbol($taxamount, $decimal);
	$json_array['total_amount'] = $general->ct_price_format_total(($final_subtotal + $taxamount) , $symbol_position, $decimal);
	$json_array['frequent_discount'] = '-' . $general->ct_price_format_without_symbol($_SESSION['freq_dis_amount'], $decimal);
	$json_array['cart_sub_total'] = $general->ct_price_format_without_symbol($total, $decimal);
	echo json_encode($json_array);
	}
...
elseif (isset($_POST['check_for_option'])){
...
}elseif (isset($_POST['add_tip_value'])){
  $jsonn_array = array();
  $_SESSION['ct_cart']['method'] = array_values($_SESSION['ct_cart']['method']);
  $c_rates = 0;
  for ($i = 0; $i < (count((array)$_SESSION['ct_cart']['method'])); $i++){
  ...
  $_SESSION['freq_dis_amount'] = $freqdis_amount;
  $allover_totals = $totals - $_SESSION['freq_dis_amount'];
  $_SESSION['tip_value'] = $_POST['tip_value'];
  $final_subtotal = $allover_totals;
  if ($settings->get_option('ct_tax_vat_status') == 'Y'){
    if ($settings->get_option('ct_tax_vat_type') == 'F'){
      $flatvalue = $settings->get_option('ct_tax_vat_value');
      $taxamount = $flatvalue;
  ...
  $alltot=($final_subtotal + $taxamount + $_POST['tip_value']);
  if($alltot <= 0){
    $alltot=0;
  }
