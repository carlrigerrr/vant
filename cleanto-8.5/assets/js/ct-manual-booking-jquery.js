...
jQuery(document).ready(function () {
    jQuery(".service-is").on("click", function (event) {
        event.stopPropagation();
        var dropdown = jQuery(".ct-services-dropdown");
        if (dropdown.hasClass("show")) {
            dropdown.hide("blind", { direction: "vertical" }, 300).removeClass("show");
        } else {
            dropdown.show("blind", { direction: "vertical" }, 300).addClass("show");
        }
    });

    jQuery(".select_service").on("click", function (event) {
        event.stopPropagation();
        jQuery("#ct_selected_service").html(jQuery(this).html());
        jQuery(".ct-services-dropdown").hide("blind", { direction: "vertical" }, 300).removeClass("show");
    });

    jQuery(document).on("click", function (event) {
        if (!jQuery(event.target).closest('.ct-services-dropdown, .service-is').length) {
            jQuery(".ct-services-dropdown").hide("blind", { direction: "vertical" }, 300).removeClass("show");
        }
    });
});
    ...
    if (existing_userid != 0) {
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                ...
                jQuery("#ct-emailss").val(userdata.email);
...
jQuery(document).on("change", ".new-user", function () {
    if (jQuery('.new-user').is(':checked')) {
        ...
        jQuery("#ct-emailss").val('');
        jQuery("#ct_email_hi").hide();
        $('#retype_status').prop('checked', false);
...
jQuery(document).on("click", "#ct_change_customer", function () {
    ...
    jQuery("#app-city").val('');
    jQuery("#app-state").val('');
    jQuery('#retype_status').prop('checked', false);
                ...
                jQuery.ajax({
                    ...
                    success: function (res) {
                        ...
                        if (search_session_data.found_status == 'found') {
                            ...
                            jQuery.ajax({
                                type: "POST",
                                // search_session_data.found_status
                                url: ajax_url + "manual_booking_frontajax.php",
                                data: {
