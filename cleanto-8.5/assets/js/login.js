jQuery(document).ready(function () {
    ...
    jQuery('#show-pass').click(function () {
        if (jQuery("#show-pass").is(":checked")) {
            jQuery('.ct-pass-check').addClass('show-pass-text');
            jQuery('.ct-pass-check').removeClass('hide-pass-text');
        } else {
            jQuery('.ct-pass-check').addClass('hide-pass-text');
            jQuery('.ct-pass-check').removeClass('show-pass-text');
        }
    });
    ...
    jQuery(document).on('click', '.mybtnloginadmin', function () {
        ...
        if (name == '') {
            jQuery('.username-blank-error').show();
            jQuery('.password-blank-error').hide();
            return false;
        }

        if (password == '') {
            jQuery('.password-blank-error').show();
            jQuery('.username-blank-error').hide();
            return false;
        }
        jQuery('.login-error').hide();
        jQuery.ajax({
    ...
    /*  INSERT Staff */
    jQuery(document).on("click", "#ct_staff_register", function (e) {
        e.preventDefault();
        ...
        if (!jQuery("#staff_registration").valid()) { return false; }
        if (jQuery("#staff_registration").valid()) {
            jQuery.ajax({
                type: "post",
                data: { "fullname": fullname, "email": email, "pass": pass, "service_ids": service_ids, action: "staff_reg" },
                url: ajax_url + "admin_login_ajax.php",
                success: function (res) {
                    jQuery(".ct-loading-main").hide();
    ...
    jQuery(document).ready(function () {
        jQuery("#staff_registration").validate({
            rules: {
                staff_email: {
                    required: true,
                    email: true,
                    remote: {
                        url: ajax_url + "admin_login_ajax.php",
                        type: "POST",
                        data: { email: function () { return jQuery("#staff_email").val(); }, },
                    }
                },
                staff_name: {
                    required: true,
