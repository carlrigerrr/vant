...
require(dirname(dirname(dirname(__FILE__))).'/SMTP/vendor/autoload.php');
...
if(isset($_POST['action']) && $_POST['action']=='update_company_setting'){
	...
	if($used_staff_images->num_rows > 0){
...
elseif(isset($_POST['action']) && $_POST['action']=='update_general_setting'){
    ...
    $ct_option=array(
      ...
      'ct_allow_user_tip'=>$_POST['ct_allow_user_tip'],
    );
...
if(isset($_POST['action']) && $_POST['action']=='email_setting'){
	if($_POST['username'] != '' && $_POST['hostname'] != '' && $_POST['password'] != ''){
		try {
			$mail->SMTPDebug = 0;
			$mail->isSMTP();
			$mail->Host = $_POST['hostname'];
			$mail->SMTPAuth = $_POST['autheticationtype'];
			$mail->Username = $_POST['username'];
			$mail->Password = $_POST['password'];
			$mail->SMTPSecure = $_POST['encryptiontype'];
			$mail->Port = $_POST['port'];

			// $mail->setFrom('from@example.com', 'Your Name');
			// $mail->addAddress('recipient@example.com', 'Recipient Name');
			$mail->setFrom($_POST['username'], 'Your Name');
			$mail->addAddress($_POST['username'], 'Recipient Name');
			...
			echo "Your SMTP credentials are wrong please verify username, hostname, password,port, Authentication type, Encryption type".$e;
			die;
		}
	}
...
if(isset($_POST['freqdis'])){
    ...
    if (isset($t_zone_value) && $t_zone_value != "") {
        $offset = $first_step->get_timezone_offset(
            $server_timezone,
            $t_zone_value
        );
        ...
        if ($one_rec_detail["stripe_plan_id"] == "") { 
            ...
            $one_product_create = $objproduct::Create([
                ...
                /* "statement_descriptor" =>
                    ...
                    $days .
                    " Days", */
                "statement_descriptor" => "Product For " . $days . " Days",
            ]);
            $objfrequently->id = $_POST["id"];
            $objfrequently->stripe_plan_id = $one_product_create->id;
            ...
            try {
                ...
                $checkProduct = $stripe->products->retrieve(
                    $one_rec_detail["stripe_plan_id"],
                    []
                );
                    
                $one_product_info_update = $objproduct::Update(
                    $one_rec_detail["stripe_plan_id"],
                    [
                        /* "statement_descriptor" =>
                            $objfrequently->discount_typename .
    ...
    if (
        ...
        try {
            ...
            $objproduct = new \Stripe\Product();
            if ($one_rec_detail["stripe_plan_id"] != "") {
                $one_product_info_retrive = $objproduct::Retrieve(
                    $one_rec_detail["stripe_plan_id"]
                );
...
if(isset($_POST['ct_create_plan']) && $_POST['ct_create_plan'] == '1') {
	if($setting->get_option('ct_stripe_payment_form_status') == "on" && $_POST['ct_stripe_create_plan'] == "Y"){
		include(dirname(dirname(dirname(__FILE__))).'/assets/stripe_3d/vendor/autoload.php');
		$secret_key = $setting->get_option('ct_stripe_secretkey');
