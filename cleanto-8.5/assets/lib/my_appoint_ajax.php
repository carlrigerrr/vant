...
$user->conn = $conn;
$objoci = new cleanto_order_client_info();
$objoci->conn = $conn;
...
$mail_s->Host = $settings->get_option('ct_smtp_hostname');
$mail_s->Username = $settings->get_option('ct_smtp_username');
$mail_s->Password = $settings->get_option('ct_smtp_password');
$mail_s->Port = $settings->get_option('ct_smtp_port');
$mail_s->SMTPSecure = $settings->get_option('ct_smtp_encryption');
...
$company_city = $settings->get_option('ct_company_city'); $company_state = $settings->get_option('ct_company_state'); $company_zip = $settings->get_option('ct_company_zip_code'); $company_country = $settings->get_option('ct_company_country'); 
...
$admin_phone_twilio = $settings->get_option('ct_sms_twilio_admin_phone_number');
$admin_phone_plivo = $settings->get_option('ct_sms_plivo_admin_phone_number');
...
if ($language_label_arr[1] != "" || $language_label_arr[3] != "" || $language_label_arr[4] != "" || $language_label_arr[5] != ""){
	...
	if($language_label_arr[3] != ''){
		...
		$label_decode_admin = 	($default_language_arr[3]);
	}
...
if(isset($_POST['getcleintdetailwith_updatereadstatus'])){
	...
	// $orderdetail = $objdashboard->getclientorder($_POST['recurring_id']);
	$recurring_id = $orderdetail[10];
            ...
            <div class="modal-content">
                ...
                <div class="modal-body mb-20">
                    <ul class="list-unstyled ct-cal-booking-details mypopupul">
						...
						<li class="<?php   if ($settings->get_option('ct_show_time_duration') == 'N') {echo "force_hidden";} ?>">
                        ...
                        if($orderdetail[4]==0)
                        {
                            ...
                            <?php 
								...
								if($settings->get_option("ct_company_willwe_getin_status") == "Y") { ?>
                            <li>
                    ...
                    <div class="cta-col12 ct-footer-popup-btn text-center">
                        ...
                        if($orderdetail[6]=='C' || $orderdetail[6]=='R' || $orderdetail[6]=='CC' || $past_day == "yes"){
						   ...
						   <?php   }
							...
							<span class="col-xs-6 np ct-w-32">
								<a data-id1="<?php echo $recurring_id;?>" id="ct-delete-appointment-cal-popup1" class="ct-delete-appointment-cal-popup btn btn-link ct-small-btn booking_deletess1" rel="popover" data-placement='top' title="<?php echo 'delete_all_appointment';?>"><i class="fa fa-trash-o fa-2x"></i><br /> <?php  echo 'delete all recurring';?></a>
							</span>
							<div id="popover-delete-appointment-cal-popupss<?php  echo $_POST['orderid'];?>" style="display: none;">
								<div class="arrow"></div>
								<table class="form-horizontal" cellspacing="0">
							...
							<div id="popover-delete-appointment-cal-popupss1<?php echo $recurring_id;?>"  style="display: none;">
								<div class="arrow"></div>
								<table class="form-horizontal" cellspacing="0">
									<tbody>
									<tr>
										<td>
											<button id="" data-id1="<?php echo $recurring_id;?>" value="Delete" class="btn btn-danger btn-sm mybtndelete_bookings" type="submit"><?php echo $label_language_values['delete'];?></button>
...
<?php 
	...
	if ($settings->get_option("ct_client_email_notification_status") == "Y" && $clientemailtemplate[4] == "E") {
	...
	if ($settings->get_option("ct_admin_email_notification_status") == "Y" && $adminemailtemplate[4] == "E") {
...
elseif(isset($_POST['confirm_booking_cal'])){
  ...
  /*SMS SENDING CODE END*/
	...
	$ct_max_advance_booking_time = $settings->get_option('ct_max_advance_booking_time');
		...
		<li id="rec-noti-1" class="notificationli" data-orderid="<?php echo $b['order_id'];?>" data-booking_status="<?php echo $b['booking_status']; ?>" style="background-color: <?php  echo $col;?>">
			<div class="list-inner">
				...
				if($b['client_id']==0)
				{
					...
					{
						$booking_stats='<span class="ct-success">'.$label_language_values['confirmed'].'</span>';
					}
			...
			<div class="modal-body">
				...
				<div class="insert_staff_cal_timeslots">
					<?php if (isset($dd['staff_ids']) && $dd['staff_ids'] != "" && $dd['staff_ids'] != 1) { 
						...
						$assigned_staff_html = "";
						foreach ($get_staff_assignid as $assignid) {
							$get_staff = $user->readone_staff($assignid);
							$assigned_staff_html .= "<p style='margin-bottom: 1px;'>: ".$get_staff['fullname']."</p>";
						}

						/*$get_staff_services = $objadmin->readall_staff_for_reschedule($get_staff['zone'], $dd['service_id']);*/
						$get_staff_services = $objadmin->readall_staff_for_reschedule($dd['service_id']);
						foreach($get_staff_services as $staff_details)
						{
							$i = "no";
							$staffname = $staff_details['fullname'];
					...
					?>
						...
						<?php 
							...
							$time_new = date("h:i A",strtotime($dd['booking_date_time']));

							$slot_time = date('H:i:s',strtotime($dd['booking_date_time']));
						...
						<div class="col-xs-12">
							<div class="form-group">
								...
								<div class="cta-col6 ct-w-50 float-right mytime_slots_booking">
									...
									$time_interval = $settings->get_option('ct_time_interval');
									$time_slots_schedule_type = $settings->get_option('ct_time_slots_schedule_type');
									$advance_bookingtime = $settings->get_option('ct_min_advance_booking_time');
									$ct_service_padding_time_before = $settings->get_option('ct_service_padding_time_before');
									$ct_service_padding_time_after = $settings->get_option('ct_service_padding_time_after');
									$booking_padding_time = $settings->get_option('ct_booking_padding_time');
									$reschedule = "Yes";
									$time_schedule = $first_step->get_day_time_slot_by_provider_id($time_slots_schedule_type,$start_date,$time_interval,$staff_id,$_POST['order_id'],$reschedule,$advance_bookingtime,$ct_service_padding_time_before,$ct_service_padding_time_after,$timezonediff,$booking_padding_time);
									$allbreak_counter = 0;
									...
									<select class="selectpicker mydatepicker_appointment   form-control" id="myuser_reschedule_time" data-size="10" style="" >
										...
										if($time_schedule['off_day']!=true && isset($time_schedule['slots']) && sizeof((array)$time_schedule['slots'])>0 && $allbreak_counter != sizeof((array)$time_schedule['slots']) && $allofftime_counter != sizeof((array)$time_schedule['slots'])){
											foreach($time_schedule['slots']  as $slot) {
												...
												if($settings->get_option('ct_hide_faded_already_booked_time_slots')=='on' && in_array($complete_time_slot,$time_schedule['booked'])) {
													continue;
												}
												if( in_array($complete_time_slot,$time_schedule['booked']) && ($settings->get_option('ct_allow_multiple_booking_for_same_timeslot_status')!='Y') ) { ?>
													<?php 
													if($settings->get_option('ct_hide_faded_already_booked_time_slots')=="on"){
														?>
														<option value="<?php echo date("H:i",strtotime($slot));?>" <?php  if(date("H:i",strtotime($slot)) == $slot_timess){ echo "selected";}?> class="time-slot br-2 ct-booked" >
															<?php 
															if($settings->get_option('ct_time_format')==24){
												...
												<?php 
													...
													?>
													<option value="<?php echo date("H:i",strtotime($slot));?>" <?php  if(date("H:i",strtotime($slot)) == $slot_timess){ echo "selected";}?> class="time-slot br-2 <?php  if(in_array($complete_time_slot,$time_schedule['booked'])){ echo' ct-booked';}else{ echo ' time_slotss'; }?>" <?php  if(in_array($complete_time_slot,$time_schedule['booked'])){echo ''; }else{ echo 'data-slot_date_to_display="'.date($getdateformate,strtotime($dates)).'" data-slot_date="'.$dates.'" data-slot_time="'.$slot_time.'"'; } ?>><?php if($settings->get_option('ct_time_format')==24){echo date("H:i",strtotime($slot));}else{echo str_replace($english_date_array,$selected_lang_label,date("h:i A",strtotime($slot)));}?></option>
												<?php 
												} $slot_counter++;
											}
...
(1).Express active in first .
(2).Order list api changes.
(3).Truck vehicle data add.

Project name:-cleanto

(1).Service unit in front view increase/decrease not show.
(2).booking details unit  issue.
(3).scrllber add the manual booking-updatebooking.
(4).property based data not display in select carpenter service.
(5). multiple qty not show in addons service.
(6).manual booking select a method   not show a active .
(7).calendar  when not show in email but multiple data add.
(8).when selecting "carpenter service" in manual booking  the value for "Area based" is not a display.
(9). service details in select in steff  there was blank space in name.
(10).add the tip in admin in manual booking.
(11).in booking details multiple services name were not display .
(12).icon  images add in unit >padding. Try it but crop image problem and logdding time add .

Today:-
- In "Booking Details," multiple methods were not visible.
- In "Payments," there was an issue with the update button and status in one column.
...
update (08-07-2024)
Project Name:- moveto
- Express active in first
- order list chages. api  
- Truck Vehicle data add.
-----------------------------------------

report(09/07/2024)
 -> project name - cleanto.

- "Service Unit Front Increase/Decrease View" do not show anything on the front end. 
- admin > Booking Details: Issue with Units Resolved, 
- Scrollbar in the corner. add and edit  Booking
- Front End: Issue with Property-Based Data Not Displaying When Selecting Carpenter Services Resolved 
- Booking date calendar show previous date 
- service manage unit  add in image 

Report (09-07-2024)

- Multiple Qty in addons service, its not working. and cant change qty in front view.  
- I am trying to add unit image to service manage unit and fix the error.

report(11/07/2024)

- While adding a manual booking, selecting the "Method" option didn't make it active. 
- In the calendar, when selecting a week, the email is not displayed correctly. 
- When selecting "Carpenter Services" in "Add Manual Booking," the value for "Area Based (Feet)" is not displayed. 
- When selecting "Service Details" under "Staff," there was a blank space appearing in the middle of the client's name. 
 
-> project name - Laundry.
- The issue with the reject button not functioning was resolved. 


report(12/07/2024)

- Admin> Add a tip colum in "Add Manual Booking.". 
- In "Booking Details," multiple service names were not visible. 
- active display in  method. with changes 

Today report(12/07/2024)
- In "Booking Details," multiple methods were not visible. > complete
- In "Payments," there was an issue with the update button and status in one column. >complete
- when i was click on reject button its going loading condition. fix the issue > complete
- in manual booking inventory title not show. > 
