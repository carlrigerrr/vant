...
if(isset($_POST['smssend'])){
	...
	$chkmsgsnd = "1";
	/* MESSAGEBIRD CODE */
	if($settings->get_option("ct_sms_messagebird_status") == "Y" && $chkmsgsnd == "1"){
	...
	/*PLIVO CODE*/
		...
		if($settings->get_option("ct_sms_telco_status") == "Y" && $chkmsgsnd == "1"){
			$telco_APIkey = $settings->get_option("ct_sms_telco_account_apikey");
			if($settings->get_option("ct_sms_telco_send_sms_to_staff_status") == "Y"){
				if(isset($staff_phone) && !empty($staff_phone)){
					...
					curl_setopt_array($curl, array(
					  CURLOPT_URL => 'https://telcosms.co.ao/send_message',
