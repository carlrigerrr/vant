...
else
{
	...
	if(isset($_POST['action']) && $_POST['action']=='display_ser_addons'){
        ...
        $cnt = 1;
		if($display_ser_addon->num_rows > 0){
			while($row=mysqli_fetch_array($display_ser_addon)){
			?>
			<tr>
				<td><?php echo $cnt;?></td>
				<td><?php if(isset($row['addon_service_name'])){ echo $row['addon_service_name']; }else{ } ?></td>											
				<td><?php echo $general->ct_price_format($row['base_price'],$symbol_position,$decimal); ?></td>
				<td><?php if(isset($row['maxqty'])){ echo $row['maxqty']; }else{ } ?></td>	
			</tr>
			<?php 
			$cnt++;}
		}
	...
	if(isset($_POST['action']) && $_POST['action']=='display_ser_method'){
        ...
        $cnt = 1;
		if($display_ser_method->num_rows > 0){
			while($row1=mysqli_fetch_array($display_ser_method)){
				if(isset($row1['status'])=='E'){
			...
			<tr>
				<td><?php echo $cnt;?></td>
				<td><?php if(isset($row1['method_title'])){ echo $row1['method_title']; }else{ } ?></td>											
	...
	if(isset($_POST['action']) && $_POST['action']=='display_booking_addons'){
        ...
        $cnt = 1;
		if($display_booking_addons->num_rows > 0){
			while($row2=mysqli_fetch_array($display_booking_addons)){
				...
				if(isset($row2['addons_service_qty'])==0){
				break;
				}
			?>
			<tr>
				...
				<td><?php if(isset($row2['addons_service_qty'])){ echo $row2['addons_service_qty']; }else{ } ?></td>
			</tr>
			<?php 
				$cnt++;
			}
	...
	if(isset($_POST['action']) && $_POST['action']=='display_booking_method'){
		...
		if($display_ser_method->num_rows > 0){
			...
			<tr>
				...
				<td><?php if(isset($row1['method_unit_qty'])){ echo $row1['method_unit_qty']; }else{ } ?></td>
