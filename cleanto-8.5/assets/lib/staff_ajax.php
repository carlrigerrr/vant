...
if (isset($_POST['u_member_email'])) {
	...
	if ($staff_register) {
		...
		if ($settings->get_option('ct_quickbooks_status') == 'Y') {
			...
			$employeobj = Employee::create([
				"GivenName" => ucwords($_POST['name']),
				"DisplayName" => ucwords($_POST['name']),
				"PrintOnCheckName" => ucwords($_POST['name']),
				"PrimaryEmailAddr" => $_POST['email'],
				"FamilyName" => ucwords($_POST['name'])
			]);
			$resultingEmployeObj = $dataService->Add($employeobj);
		}
		/** Quickbooks Code End **/
	}
	...
	<div class="ct-staff-details tab-content col-md-9 col-sm-8 col-lg-9 col-xs-12">
		...
		<div class="ct-staff-top-header">
			...
			<button id="ct-delete-staff-member" class="pull-right btn btn-circle btn-danger" rel="popover"
				data-placement='left' title="<?php echo $label_language_values['delete_member']; ?>"> <i
			...
			<div id="popover-delete-member" style="display: none;">
				...
				<table class="form-horizontal" cellspacing="0">
					<tbody>
						<tr>
							<td>
								<button id="" data-id="<?php echo $staff_id; ?>" value="Delete"
									class="staff_delete btn btn-danger"
									type="submit"><?php echo $label_language_values['yes']; ?></button>
								<button id="ct-close-popover-delete-staff" class="btn btn-default"
									href="javascript:void(0)"><?php echo $label_language_values['cancel']; ?></button>
							</td>
						</tr>
					</tbody>
				</table>
				...
				<div class="tab-pane col-lg-12 col-md-12 col-sm-12 col-xs-12 active" id="member-details">
					<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
						<div class="ct-clean-service-image-uploader">
							...
							?>
							<img data-imagename="" id="pppp<?php echo $staff_read['id']; ?>staffimage"
								src="<?php echo $imagepath; ?>" class="ct-clean-staff-image br-100" height="100"
								width="100">
							<input data-us="pppp<?php echo $staff_read['id']; ?>" class="hide ct-upload-images" type="file"
								name="" id="ct-upload-imagepppp<?php echo $staff_read['id']; ?>"
								data-id="<?php echo $staff_read['id']; ?>" />
							<?php
							if ($staff_read['image'] == '') {
								?>
								<label for="ct-upload-imagepppp<?php echo $staff_read['id']; ?>"
									class="ct-clean-staff-img-icon-label old_cam_ser<?php echo $staff_read['id']; ?>">
									<i class="ct-camera-icon-common br-100 fa fa-camera"
										id="pcls<?php echo $staff_read['id']; ?>camera"></i>
									<i class="pull-left fa fa-plus-circle fa-2x"
										id="ctsc<?php echo $staff_read['id']; ?>plus"></i>
								</label>
								<?php
							}
							?>
							...
							<label for="ct-upload-imagepppp<?php echo $staff_read['id']; ?>"
								class="ct-clean-staff-img-icon-label new_cam_ser ser_cam_btn<?php echo $staff_read['id']; ?>"
								id="ct-upload-imagepppp<?php echo $staff_read['id']; ?>" style="display:none;">
								<i class="ct-camera-icon-common br-100 fa fa-camera"
									id="pppp<?php echo $staff_read['id']; ?>camera"></i>
								<i class="pull-left fa fa-plus-circle fa-2x"
									id="ctsc<?php echo $staff_read['id']; ?>plus"></i>
							</label>
							<?php
							if ($staff_read['image'] !== '') {
								?>
								<a id="ct-remove-staff-imagepppp<?php echo $staff_read['id']; ?>"
									data-pclsid="<?php echo $staff_read['id']; ?>"
									data-staff_id="<?php echo $staff_read['id']; ?>"
									class="delete_staff_image pull-left br-100 btn-danger bt-remove-staff-img btn-xs ser_new_del<?php echo $staff_read['id']; ?>"
									rel="popover" data-placement='left'
									title="<?php echo $label_language_values['remove_image']; ?>"> <i class="fa fa-trash"
							...
							<div id="popover-ct-remove-staff-imagepppp<?php echo $staff_read['id']; ?>"
								style="display: none;">
								<div class="arrow"></div>
								<table class="form-horizontal" cellspacing="0">
									<tbody>
										<tr>
											<td>
												<a href="javascript:void(0)" id="staff_del_images" value="Delete"
													data-staff_id="<?php echo $staff_read['id']; ?>"
													class="btn btn-danger btn-sm"
													type="submit"><?php echo $label_language_values['yes']; ?></a>
												<a href="javascript:void(0)" id="ct-close-popover-staff-image"
													class="btn btn-default btn-sm"
													href="javascript:void(0)"><?php echo $label_language_values['cancel']; ?></a>
											</td>
										</tr>
									</tbody>
								</table>
						...
						<div id="ct-image-upload-popuppppp<?php echo $staff_read['id']; ?>"
									...
									<div class="modal-content">
										<div class="modal-header">
											<div class="col-md-12 col-xs-12">
												<a data-staff_id="<?php echo $staff_read['id']; ?>"
													data-us="pppp<?php echo $staff_read['id']; ?>"
													class="btn btn-success ct_upload_img_staff"
													data-imageinputid="ct-upload-imagepppp<?php echo $staff_read['id']; ?>"
													data-id="<?php echo $staff_read['id']; ?>"><?php echo $label_language_values['crop_and_save']; ?></a>
												<button type="button" class="btn btn-default hidemodal" data-dismiss="modal"
													aria-hidden="true"><?php echo $label_language_values['cancel']; ?></button>
											</div>
										</div>
										<div class="modal-body">
											<img id="ct-preview-imgpppp<?php echo $staff_read['id']; ?>"
												style="width: 100%;" />
										</div>
												...
												<div class="col-md-12 np">
													<div class="col-md-4 col-xs-12">
														...
														<input type="text" class="form-control"
															id="ppppfilesize<?php echo $staff_read['id']; ?>"
															name="filesize" />
													</div>
													<div class="col-md-4 col-xs-12">
														<label class="pull-left">H</label> <input type="text"
															class="form-control" id="pppp<?php echo $staff_read['id']; ?>h"
															name="h" />
													</div>
													<div class="col-md-4 col-xs-12">
														<label class="pull-left">W</label> <input type="text"
															class="form-control" id="pppp<?php echo $staff_read['id']; ?>w"
															name="w" />
													</div>
													<!-- hidden crop params -->
													<input type="hidden" id="pppp<?php echo $staff_read['id']; ?>x1"
														name="x1" />
													<input type="hidden" id="pppp<?php echo $staff_read['id']; ?>y1"
														name="y1" />
													<input type="hidden" id="pppp<?php echo $staff_read['id']; ?>x2"
														name="x2" />
													<input type="hidden" id="pppp<?php echo $staff_read['id']; ?>y2"
														name="y2" />
													<input type="hidden" id="pppp<?php echo $staff_read['id']; ?>id"
														name="id" value="<?php echo $staff_read['id']; ?>" />
													<input id="ppppctimage<?php echo $staff_read['id']; ?>" type="hidden"
														name="ctimage" />
													<input type="hidden" id="recordid"
														value="<?php echo $staff_read['id']; ?>">
													<input type="hidden"
														id="pppp<?php echo $staff_read['id']; ?>ctimagename" class="ppppimg"
														name="ctimagename" value="<?php echo $staff_read['image']; ?>" />
													<input type="hidden" id="pppp<?php echo $staff_read['id']; ?>newname"
														value="staff_" />
												</div>
											</div>
										</div>
					...
					<div class="col-lg-10 col-md-10 col-sm-10 col-xs-12 mt-10">
						<form id="staff_update_details">
							<div class="ct-staff-common-table ct-staff-common-table1">
								...
								<div class="form-group col-xs-12 col-md-12">
									<div class="col-xs-4 col-md-2"><label
											for="ct-member-name"><?php echo $label_language_values['name']; ?></label></div>
									<div class="col-xs-8 col-md-10"><input type="text" class="form-control"
											id="ct-member-name" value="<?php echo $staff_read['fullname']; ?>"
											name="u_member_name" /></div>
								</div>

								<div class="form-group col-xs-12 col-md-12">
									<div class="col-xs-4 col-md-2"><label
											for="ct-member-name"><?php echo $label_language_values['email'] . " " . $label_language_values['address']; ?></label>
									</div>
									<div class="col-xs-8 col-md-10"><input type="text" class="form-control"
											id="ct-member-email" value="<?php echo $staff_read['email']; ?>"
								...
								<div class="form-group col-xs-12 col-md-12">
									...
									<div class="col-xs-8 col-md-10"><textarea class="form-control" id="ct-member-desc"
											name="ct-member-desc"><?php echo $staff_read['description']; ?></textarea></div>
								</div>

								<div class="form-group col-xs-12 col-md-12">
									<div class="col-xs-4 col-md-2"><label
											for="phone-number"><?php echo $label_language_values['phone']; ?> </label></div>
									<div class="col-xs-8 col-md-10"><input type="tel" class="form-control" id="phone-number"
											name="phone-number" value="<?php echo $staff_read['phone']; ?>"
											onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" />
									</div>
								</div>

								<div class="form-group col-xs-12 col-md-12">
									<div class="col-xs-4 col-md-2"><label for="address"><?php echo "address"; ?></label>
									</div>
									<div class="col-xs-8 col-md-10"><input type="text" class="form-control"
											name="ct-member-address" id="ct-member-address"
											placeholder="Member Street Address"
											value="<?php echo $staff_read['address']; ?>" />
									</div>
								</div>
								...
								<div class="col-xs-12">
									<div class="col-xs-12 col-md-6 form-group npl npr">
										<div class="col-xs-4"><label
												for="city"><?php echo $label_language_values['city']; ?></label></div>
										<div class="col-xs-8">
											<input class="form-control value_city" placeholder="City" id="ct-member-city"
												name="ct-member-city" value="<?php echo $staff_read['city']; ?>"
												type="text">
										</div>
									</div>
									...
									<div class="col-xs-12 col-md-6 form-group npl npr">
										<div class="col-xs-4"><label
												for="state"><?php echo $label_language_values['state']; ?></label></div>
										<div class="col-xs-8">
											<input class="form-control value_state" id="ct-member-state" placeholder="State"
												name="ct-member-state" type="text"
												value="<?php echo $staff_read['state']; ?>">
										</div>
									</div>
								</div>
								...
								<div class="col-xs-12">
									<div class="col-xs-12 col-md-6 form-group npl npr">
										<div class="col-xs-4"><label
												for="zip"><?php echo $label_language_values['zip']; ?></label></div>
										<div class="col-xs-8">
											<input class="form-control value_zip" placeholder="Zip" id="ct-member-zip"
												name="ct-member-zip" type="text" value="<?php echo $staff_read['zip']; ?>">
										</div>
									</div>

									<div class="col-xs-12 col-md-6 form-group npl npr">
										<div class="col-xs-4"><label
												for="country"><?php echo $label_language_values['country']; ?></label></div>
										<div class="col-xs-8">
											<input class="form-control value_country" placeholder="Country"
												id="ct-member-country" name="ct-member-countrys" type="text"
												value="<?php echo $staff_read['country']; ?>">
										</div>
									</div>
								</div>
							...
							<div class="col-xs-12">
								<div class="col-xs-4 col-md-2"><label
										for="enable-booking1"><?php echo $label_language_values['enable_booking']; ?></label>
								</div>
								<div class="col-xs-8 col-md-10">
									<label for="enable-booking1">
										<input type="checkbox" id="enable-booking1" data-toggle="toggle" data-size="small"
											data-on="<?php echo $label_language_values['yes']; ?>" <?php if ($staff_read['enable_booking'] == "Y") {
												   echo "checked";
											   } ?> data-off="<?php echo $label_language_values['no']; ?>" data-onstyle="success"
							...
							<div class="col-xs-12">
								<div class="col-xs-4 col-md-2"></div>
								<div class="col-xs-8 col-md-10">
									<a href="javascript:void(0);" id="update_staff_details"
										data-old_schedule_type="<?php echo $staff_read['schedule_type']; ?>"
										class="btn btn-success ct-btn-width mt-20"
										data-id="<?php echo $staff_read['id']; ?>"
										type="submit"><?php echo $label_language_values['save']; ?></a>
								</div>
							</div>
				...
				<div class="tab-pane member-service-details" id="member-service-details">
					<div class="panel panel-default">
						<div class="table-responsive">
							<table id="ct-staff-service-details-list"
								class="display responsive nowrap table table-striped table-bordered" cellspacing="0"
								width="100%">
								<thead>
									<th>#</th>
										...
										foreach ($staff_service_details as $arr_staff) {
											...
											<tr>
												...
												<td>
													...
													$p_client_name_res = str_split($p_client_name);
												...
												<td><?php echo str_replace($english_date_array, $selected_lang_label, date($getdateformat, strtotime($arr_staff['booking_date_time']))); ?>
												</td>
												<td><?php echo str_replace($english_date_array, $selected_lang_label, date($timess, strtotime($arr_staff['booking_date_time']))); ?>
												</td>
												<td><?php echo $general->ct_price_format($get_booking_nettotal, $symbol_position, $decimal); ?>
												</td>
											</tr>
											<?php
										}
										...
										<div class="panel panel-default">
												...
												<ul class="nav nav-pills nav-stacked">
													...
													if ($minweek == 1 && $maxweek == 5) {
														for ($i = $minweek; $i <= $maxweek; $i++) {
															...
															<li class="<?php if ($i == 1) {
																echo "active";
															} ?>"><a href="#<?php echo $weeknameid[$i - 1]; ?>"
																	data-toggle="tab"><?php echo $weeknameid[$i - 1]; ?> </a></li>
															<?php
														}
													} else {
														$i = 1; ?>
														<li class="<?php if ($i == 1) {
															echo "active";
																...
																data-toggle="tab"><?php echo $label_language_values['this_week']; ?></a>
														</li>
														<?php
													}
													?>
											...
											<div class="col-sm-9 col-md-9 col-lg-9 col-xs-12">
												...
												<div class="tab-content">
													<span class="prove_schedule_type"
														style="visibility: hidden;"><?php echo $option[7]; ?></span>
													<?php
													for ($i = $minweek; $i <= $maxweek; $i++) {
														?>
															...
															<div class="panel panel-default">
																<div class="panel-body">
																	<?php if ($minweek == 1 && $maxweek == 1) { ?>
																		<h4 class="ct-right-header">
																			<?php echo $label_language_values['this_week_time_scheduling']; ?>
																		</h4>
																		<?php
																	} else {
																		?>
																		<h4 class="ct-right-header">
																			<?php echo $weekname[$i - 1]; ?>			<?php echo " " . $label_language_values['week_time_scheduling']; ?>
																		</h4>
																		<?php
																	} ?>
																		...
																		for ($j = 1; $j <= 7; $j++) {
																			...
																			<li class="active">
																				...
																				<span class="col-sm-2 col-md-2 col-lg-2 col-xs-12">
																					<label class="cta-col2"
																						for="ct-monFirst<?php echo $i; ?><?php echo $j; ?>_<?php echo $getvalue[0]; ?>">
																				...
																				<span
																					...
																					<div class="pull-right">
																						...
																						?>
																						<span class="ct-staff-hours-to">
																							<?php echo $label_language_values['to']; ?>
																						</span>
																						<?php
																						if ($time_format == 24) {
												...
												<ul class="nav nav-pills nav-stacked">
													...
													if ($minweek == 1 && $maxweek == 5) {
														for ($i = $minweek; $i <= $maxweek; $i++) {
															...
															<li class="<?php if ($i == 1) {
																echo "active";
															} ?>"><a href="#<?php echo $breaks_weeknameid[$i - 1] . "_br"; ?>"
																	data-toggle="tab"><?php echo $breaks_weeknameid[$i - 1]; ?> </a>
															</li>
															<?php
														}
													} else {
														...
														<div class="tab-pane <?php if ($i == 1) {
															...
															<div class="panel panel-default">
																<div class="panel-body">
																	<?php if ($minweek == 1 && $maxweek == 1) { ?>
																		<h4 class="ct-right-header">
																			<?php echo $label_language_values['this_week_breaks']; ?>
																		</h4>
																		<?php
																	} else { ?>
																		<h4 class="ct-right-header">
																			<?php echo $breaks_weekname[$i - 1]; ?>			<?php echo $label_language_values['week_breaks']; ?>
																		</h4>
																		<?php
																	} ?>
																		...
																		for ($j = 1; $j <= 7; $j++) {
																			...
																			<li class="active">
																				...
																				if ($getdatafrom_week_days[0] == 'Y' || $getdatafrom_week_days[0] == '') {
																					...
																					<span class="col-sm-2 col-md-2 col-lg-2 col-xs-12">
																						<a
																							class="btn btn-small btn-default ct-small-br-btn disabled"><?php echo $label_language_values['closed']; ?></a>
																					</span>
																					<?php
																				} else { ?>
																					<span class="col-sm-2 col-md-2 col-lg-2 col-xs-12">
																				...
																				<span
																					...
																					<ul class="list-unstyled"
																						id="ct-add-break-ul<?php echo $i; ?>_<?php echo $j; ?>">
																						<?php
																						...
																						while ($rrr = mysqli_fetch_array($jc)) {
																							...
																							<li>
																								...
																								<div id="popover-delete-breaks<?php echo $rrr['id']; ?>_<?php echo $i; ?>_<?php echo $j; ?>"
																									style="display: none;">
																									<div class="arrow"></div>
																										...
																										<tbody>
																											<tr>
																												<td>
																													<button id=""
																														value="Delete"
																														data-break_id='<?php echo $rrr['id']; ?>'
																														class="btn btn-danger mybtndelete_breaks"
																														type="submit"><?php echo $label_language_values['yes']; ?></button>
																													<button
																														id="ct-close-popover-delete-breaks"
																														class="btn btn-default close_popup"
																														href="javascript:void()"><?php echo $label_language_values['cancel']; ?></button>
																												</td>
																											</tr>
																										</tbody>
																									</table>
								...
								<div class="tab-pane member-offtime" id="member-offtime">
									<div class="panel panel-default">
										<div class="panel-body">
											...
											<div
												class="ct-staff-member-offtime-list-main mytablefor_offtimes cb col-md-12 col-xs-12">
												<div class="table-responsive">
															...
															if ($res->num_rows > 0) {
																while ($r = mysqli_fetch_array($res)) {
																	...
																	<tr id="myofftime_<?php echo $r['id'] ?>">
																		<td><?php echo $i++; ?></td>
																		<td><?php echo str_replace($english_date_array, $selected_lang_label, date($getdateformat, strtotime($sdates))); ?>
																		</td>
																		<?php
																		if ($time_format == 12) {
																			?>
																			<td><?php echo str_replace($english_date_array, $selected_lang_label, date("h:i A", strtotime($stime))); ?>
																			</td>
																			<?php
																		} else {
																			?>
																		...
																		?>
																		<td><?php echo str_replace($english_date_array, $selected_lang_label, date($getdateformat, strtotime($edates))); ?>
																		</td>
																		<?php
																		if ($time_format == 12) {
																			?>
																			<td><?php echo str_replace($english_date_array, $selected_lang_label, date("h:i A", strtotime($etime))); ?>
																			</td>
																			<?php
																		} else {
																			?>
				...
				if ($settings->get_option('ct_sms_twilio_send_sms_to_staff_status') == "Y") {
					if (isset($staff_phone) && !empty($staff_phone)) {
						...
						if ($template[4] == "E") {
							...
							$client_sms_body = str_replace($searcharray, $replacearray, $message);
							/*TWILIO CODE*/
							$message = $twilliosms_client->account->messages->create(
								array(
...
if (isset($_POST['get_staff_bookingandpayment_by_dateser'])) {
	...
	?>
	<table id="payments-staff-bookingandpymnt-details-ajax"
			...
			if (mysqli_num_rows($all_bookings) > 0) {
				while ($all = mysqli_fetch_array($all_bookings)) {
					...
					<tr>
						...
						<td><?php echo $service_name; ?></td>
						<td><?php echo str_replace($english_date_array, $selected_lang_label, date($getdateformat, strtotime($all['booking_date_time']))); ?>
						</td>
...
if (isset($_POST['get_payment_staff_by_date'])) {
	...
	?>
	<table id="payments-staffp-details-ajax" class="display responsive nowrap table table-striped table-bordered"
		cellspacing="0" width="100%">
		<thead>
			<tr>
				<th>#</th>
			...
			if (mysqli_num_rows($all_bookings) > 0) {
				...
				while ($row = mysqli_fetch_array($all_bookings)) {
					...
					<tr>
						...
						<td><?php echo str_replace($english_date_array, $selected_lang_label, date($getdateformat, strtotime($row['payment_date']))); ?>
						</td>
...
if (isset($_POST['sendotp'])) {
	...
	$sms_countrycode_change = isset($_SESSION['sms_countrycode_change']) ? $_SESSION['sms_countrycode_change'] : "";

	if ($sms_countrycode_change == "log") {
		$objadminprofile->phoneno = $_POST['phoneno'];
	...
	if ($checkotp && isset($checkotp[0]) && $checkotp[0] != '') {
	...
	if ($settings->get_option("ct_sms_twilio_status") == "Y") {
		...
		$message = $obj_twillio->account->messages->create(
			array(
	...
	if ($settings->get_option("ct_nexmo_status") == "Y") {
		...
		$message = "Your One time Password for Registration is " . "" . $otp . ".Please Verify Once For Registration.";
		$res = $obj_nexmo->send_nexmo_sms($staff_phone, $message);
	}
	echo json_encode($staff_phone);
}
/* Send OTP action end Jay W */

if (isset($_POST['action']) && $_POST['action'] == 'get_lat_lon') {

	$google_api_key = $settings->get_option('ct_google_api_key');
	...
	if ($google_api_key != '') {
		...
		$address = $get_address . ', ' . $city . ', ' . $state . ', ' . $country;

		$formattedAddr = str_replace(' ', '+', $address);
		//Send request and receive json data by address
		$geocodeFromAddr = file_get_contents('https://maps.googleapis.com/maps/api/geocode/json?address=' . $formattedAddr . '&sensor=false&key=' . $google_api_key);
