    ...
    while($cc = mysqli_fetch_array($clientbooking)){
        ...
        <tr>
           ...
           <?php 
            ...
            <td><?php  if(isset($cc['pna']) != 0){ echo $cc['c_payment_method'];} else { echo "Free";};?></td>
            <td>
                <?php 
                /* methods */
	...
	if($clientbooking->num_rows > 0){
        while($cc = mysqli_fetch_array($clientbooking)){
            ...
            <tr>
                ...
                <td><?php if(isset($cc['sname'])){ echo $cc['sname']; }else{ }?></td>
                <?php 
				if($time_format == 12){
				?>
				<td><?php echo str_replace($english_date_array,$selected_lang_label,date("".$getdateformat." h:i A",strtotime($cc['booking_date_time'])));?></td>
