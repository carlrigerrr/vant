	...
	if($res->num_rows > 0){
		while ($arrs = mysqli_fetch_array($res)) {
			...
			<div class="col-sm-12 col-md-12 col-xs-12">
				<li class="panel panel-default ct-clean-services-panel mysortlist_methods" data-id="<?php if(isset($arrs['id'])){ echo $arrs['id']; }else{ }?>" id="service_method_<?php  if(isset($arrs['id'])){ echo $arrs['id']; }else{ }?>">
					<div class="panel-heading">
						<h4 class="panel-title">
							<div class="cta-col8 col-sm-6 np">
								...
								<span class="ct-clean-service-title-name" id="service_name<?php  if(isset($arrs['id'])){ echo $arrs['id']; }else{ }?>"><?php echo $arrs['method_title'];?></span>
							</div>
							<div class="pull-right cta-col4 col-sm-6 np">
								<div class="cta-col4 cta-unit-pricing">
									<a data-id="<?php if(isset($arrs['id'])){ echo $arrs['id']; }else{ }?>" data-name="<?php echo $arrs['method_title'];?>"
								...
								<div class="cta-col4 cta-up-endis ta-r">
									<label for="sevice-endis-<?php if(isset($arrs['id'])){ echo $arrs['id']; }else{ }?>">
										<input class='myservices_methods_status' data-toggle="toggle" data-size="small" type='checkbox' data-id="<?php if(isset($arrs['id'])){ echo $arrs['id']; }else{ }?>" <?php  if ($arrs['status'] == 'E') { echo "checked"; } else { echo ""; }?> id="sevice-endis-<?php if(isset($arrs['id'])){ echo $arrs['id']; }else{ }?>" data-on="<?php echo $label_language_values['enable'];?>" data-off="<?php echo $label_language_values['disable'];?>" data-onstyle='success' data-offstyle='danger' />
									</label>
								</div>
								<div class="pull-right cta-unit-del-toggle">
									...
									{
										?>
										<a id="ct-delete-service-method<?php if(isset($arrs['id'])){ echo $arrs['id']; }else{ }?>" data-toggle="popover"
										...
										<div id="popover-delete-service" style="display: none;">
											...
											<table class="form-horizontal" cellspacing="0">
												...
												<tr>
													<td>
														<button data-servicemethodid="<?php if(isset($arrs['id'])){ echo $arrs['id']; }else{ }?>" value="Delete" class="btn btn-danger btn-sm service-methods-delete-button" type="submit"><?php echo $label_language_values['yes'];?></button>
														<button id="ct-close-popover-delete-service-method" class="btn btn-default btn-sm" href="javascript:void(0)" data-servicemethodid="<?php if(isset($arrs['id'])){ echo $arrs['id']; }else{ }?>"><?php echo $label_language_values['cancel'];?></button>
													</td>
												</tr>
												</tbody>
											</table>
					...
					<div id="detailmes_sp<?php  if(isset($arrs['id'])){ echo $arrs['id']; }else{ }?>" class="servicemeth_details panel-collapse collapse">
					<div class="panel-body">
							<div class="ct-service-collapse-div col-sm-7 col-md-7 col-lg-7 col-xs-12">
								<form id="service_method_editform<?php  if(isset($arrs['id'])){ echo $arrs['id']; }else{ }?>" method="post" type="" class="slide-toggle">
									<table class="ct-create-service-table">
										<tbody>
										<tr>
											<td><label for=""><?php echo $label_language_values['method_name'];?></label></td>
											<td>
												<div class="col-xs-12 col-sm-11"><input type="text" id="txtedtmethodname<?php  if(isset($arrs['id'])){ echo $arrs['id']; }else{ }?>" class="form-control mytxtservice_methodname<?php  if(isset($arrs['id'])){ echo $arrs['id']; }else{ }?>" value="<?php echo $arrs['method_title']?>"/>
												</div>
											</td>
											<td>
												<a data-id="<?php if(isset($arrs['id'])){ echo $arrs['id']; }else{ }?>" name="" class="btn btn-success ct-btn-width btnservices_method_update"><?php echo $label_language_values['update'];?></a>
											</td>
										</tr>
										</tbody>
									</table>
...
elseif (isset($_POST['action']) && $_POST['action']=='deletemethod') {
	...
	if($methods_units->num_rows > 0){
		while($methods_units_arr = mysqli_fetch_array($methods_units))
		{
			...
			if($methods_units_rate->num_rows > 0){
