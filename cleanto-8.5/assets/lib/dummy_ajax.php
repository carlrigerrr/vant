...
/*===================================== add first services ========================================*/
		...
		$secondary_color = $setting->get_option('ct_secondary_color');

		if($sample_data_status!="Y"){
		$s1->s_title = 'House Cleaning';
		$s1->s_description = '<strong style="color:'.$secondary_color.'">House Cleaners</strong> work in residential settings where they are expected to keep <u>houses clean and well-organized.</u>';
		$s1->s_color = '#00a87e';
		$s1->s_image = '';
		$s1->s_status = 'E';
		$s1->s_position = '0';
		...
		if($add_service1){
			...
			$add_services_method1_id = $add_services_method1;
			...
			/*add first unit */
				...
				$s1->smu_units_title = 'Bedroom(s)';
				$s1->smu_base_price = '10';
				$s1->smu_minlimit = '1';
				$s1->smu_maxlimit = '10';
				$s1->smu_status = 'E';
				$s1->smu_limit_title = 'Select Bedroom(s)';
				$s1->smu_unit_symbol = '';
				$s1->smu_uduration = '30';
				$s1->sa_predefine_image = '';
				$s1->sa_predefine_image_title = '';
				$s1->sa_image = '';
				$add_services_method_unit1 = $s1->add_services_method_unit();
				$add_services_method_unit1_id = $add_services_method_unit1;
			/*add second unit */
				$s1->smu_service_id = $add_service1;
				$s1->smu_methods_id = $add_services_method1;
				$s1->smu_units_title = 'Bathroom(s)';
				$s1->smu_base_price = '12';
				$s1->smu_minlimit = '1';
				$s1->smu_maxlimit = '8';
				$s1->smu_status = 'E';
				$s1->smu_limit_title = 'Select Bathroom(s)';
				$s1->smu_unit_symbol = '';
				$s1->smu_uduration = '30';
				$s1->sa_predefine_image = '';
				$s1->sa_predefine_image_title = '';
			...
			$add_services_addon1_id = $add_services_addon1;
...
/*====================================== add second services =========================================*/
		...
		$s1->s_description = 'We are ready to provide <strong style="color:'.$secondary_color.'">Plumbing Services</strong> you can <u>trust!</u>';
		$s1->s_color = '#006999';
		$s1->s_image = '';
		$s1->s_status = 'E';
		$s1->s_position = '1';
...
/*============================================ add third services =========================================*/
		...
		$s1->s_description = 'We are ready to <strong style="color:'.$secondary_color.'">Carpenter Services</strong> you can <u>trust!</u>';
		$s1->s_color = '#996600';
		$s1->s_image = '';
		$s1->s_status = 'E';
		$s1->s_position = '2';
		...
		if($add_service3){
			...
			/* add second service method */
			$s1->sm_service_id = $add_service3;
			$s1->sm_method_title = 'Area Based (Feet)';
			$s1->sm_status = 'E';
			$add_services_method2 = $s1->add_services_method();
			if($add_services_method2){
			/*add units design */
			...
			/*add first unit */
				...
				$s1->smu_status = 'E';
				$s1->smu_limit_title = 'Enter Area of Cleaning';
				$s1->smu_unit_symbol = 'sq. ft.';
				$s1->smu_uduration = '30';
				$s1->sa_predefine_image = '';
				$s1->sa_predefine_image_title = '';
				$s1->sa_image = '';
...
/*============================================ add fourth services =========================================*/
		...
		$s1->s_description = '<strong style="color:'.$secondary_color.'">Office/Commercial</strong> Cleaners clean offices, industrial work areas, and other premises using heavy duty <u>cleaning equipment.</u>';
		$s1->s_color = '#800080';
		$s1->s_image = '';
		$s1->s_status = 'E';
		$s1->s_position = '3';
...
/************************** Add Off Day *****************************/
		...
		$dummy->u_cus_dt = date("Y-m-d H:i:s");
		$dummy->u_stripe_id = "";
		...
		if($add_user){
			...
			$dummy->b_method_id = $add_services_method1_id;
			$dummy->b_method_unit_id = $add_services_method_unit1_id;
			...
			$dummy->ba_addons_service_id = $add_services_addon1_id;
			$dummy->ba_addons_service_qty = 5;
			$dummy->ba_addons_service_rate = 25;
			...
			$dummy->oci_client_personal_info = base64_encode(serialize(array('zip'=>'90001','address'=>'Perrine, USA','city'=>'Perrine','state'=>'USA','notes'=>'Happy Booking','vc_status'=>'Y','p_status'=>'Y','contact_status'=>'Please call me')));
			$dummy->oci_order_duration = 240;
			$dummy->oci_reccuring_id = 1;
...
elseif(isset($_POST['remove_sample_data'])){
