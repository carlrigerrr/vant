...
if($reg_user_data->num_rows > 0){
	while($r_data = mysqli_fetch_array($reg_user_data)){
		?>
		<input type="hidden" id="proid<?php   echo $r_data['id']; ?>" value="<?php echo $r_data['id']; ?>" />
		<input type="hidden" id="prodt<?php   echo $r_data['id']; ?>" value="<?php if(isset($r_data['first_name']) && isset($r_data['last_name'])){
			echo $r_data['first_name']." ".$r_data['last_name'];
		} ?>" />
		<?php  
	}
}
...
#cta div.dataTables_wrapper div.dataTables_paginate
{
    float: right;
    z-index: 99;
...
#cta-customers-listing div.dt-buttons, #cta #registered-client-booking-details_wrapper div.dt-buttons
{
    text-align: center;
    float: none;
				...
				<div class="tab-content">
					<div id="registered-customers-listing" class="tab-pane fade in active">
						...
						<div id="accordion" class="panel-group">
							<table id="post_list" class="display responsive nowrap table table-striped table-bordered reg-cust-tbl" cellspacing="0" width="100%">
								<thead>
								<tr>
									<th><?php echo $label_language_values['customer']." ".$label_language_values['name'];?></th>
									<th><?php echo $label_language_values['email']." ".$label_language_values['address'];?></th>
...
<a href="emlsms.php" class="btn btn-primary pull-right col-xs-5 m-15-50"><?php echo $label_language_values['all_messages'];?></a>
					...
					<div id="guest-customers-listing" class="tab-pane fade">
						...
						<div id="accordion" class="panel-group">
							<table id="guest-client-table" class="display responsive nowrap table table-striped table-bordered" cellspacing="0" width="100%">
								...
								if($guest_user_data->num_rows > 0){

									while($g_data = mysqli_fetch_array($guest_user_data)){
										?>
										<tr id="myguest_<?php if(isset($g_data['order_id']) != ''){echo $g_data['order_id'];}else{echo 'N/A';};?>">
											<td><?php if(isset($g_data['client_name']) != ''){echo $g_data['client_name'];}else{echo 'N/A';} ?></td>
											<td><?php if(isset($g_data['client_email']) != ''){echo $g_data['client_email'];}else{echo 'N/A';} ?></td>
											<td><?php if(strlen($g_data['client_phone'])>6){echo $g_data['client_phone'];}else{echo 'N/A';} ?></td>
											<td class="ct-bookings-td">
												<a class="btn btn-primary myguestcust_bookings" data-email="<?php if(isset($g_data['client_email']) != ''){echo $g_data['client_email'];}else{echo 'N/A';} ?>" href="#guest-details" data-toggle="modal" data-id="<?php if(isset($g_data['order_id']) != ''){echo $g_data['order_id'];}else{echo 'N/A';};?>">
													<?php   echo $label_language_values['bookings'];?>
													
												</a>
												<a data-id="<?php if(isset($g_data['order_id']) != ''){echo $g_data['order_id'];}else{echo 'N/A';};?>" class="btn btn-danger col-sm-offset-1 mybtndelete_guest_customers_entry"><i class="fa fa-trash"></i> <?php   echo $label_language_values['delete'];?></a>
											</td>
										</tr>
									<?php  
									}
			...
			<div class="tab-content">
				<div id="email_add" class="tab-pane fade in active">
					...
					<form id="eml" enctype="multipart/form-data">
						...
						<div class="form-field form-required col-xs-12 mb-15">
							<label class="col-xs-12 col-md-2 control-label"><?php echo $label_language_values['subject'];?></label>
							<div class="col-xs-12 col-md-10 "><input type="text" class="form-control" id="email_sub" required="required" /></div>
						</div>
						<div class="form-field form-required col-xs-12 mb-15">
							<label class="col-xs-12 col-md-2 control-label"><?php echo $label_language_values['message'];?></label>
						<div class="col-xs-12 col-md-10"><textarea id="email_msg" name="textarea" class="col-xs-8 jqte-test"></textarea>
							</div>
						</div>
						<div class="form-field form-required col-xs-12 mb-15">
							<label class="col-xs-12 col-md-2 control-label"><?php echo $label_language_values['add_attachment'];?></label>
						...
						<div class="form-field form-required col-xs-12 mb-10">
							...
							<div class="col-xs-10 "><label class="col-xs-2 eml_errors error" style="bottom: 12px; right: 15px;"></label><a class="btn btn-success" id="eml_sub_add"><i class=""></i><?php echo $label_language_values['send'];?></a></div>
						</div>
				...
				<div id="sms_add" class="tab-pane fade">
					...
					<form id="sms" enctype="multipart/form-data">
						...
						<div class="form-field form-required col-xs-12 mb-15">
							<label class="col-xs-12 col-md-2 control-label"><?php echo $label_language_values['message'];?></label>
							<div class="col-xs-12 col-md-10 "><textarea id="sms_msg" class="form-control"></textarea></div>
						</div>
						<!-- java script badhu baki -->
						<div class="form-field form-required col-xs-12 mb-10">
							<label class="col-xs-12 col-md-2"></label>
							<div class="col-xs-12 col-md-10"><label class="col-xs-2 sms_errors error"></label><a class="btn btn-success" id="sms_sub_add"><i class=""></i><?php echo $label_language_values['send'];?></a></div>
						</div>
	...
	<div class="modal fade reg-cust-mdl" id="delete_new_customer" role="dialog">
      ...
      <div class="modal-content">
		...
		<div class="modal-footer">
          <a value="Delete" class="btn btn-danger btn-sm mybtndelete_register_customers_entry"><?php echo $label_language_values['yes'];?></a>
					<button id="ct-close-popover-customerss" data-dismiss="modal" class="btn btn-default btn-sm no-btn" href="javascript:void(0)"><?php echo $label_language_values['no'];?></button>
        </div>
		...
		<form class="add_new_user_add" id="add_new_user_add">
			<div class="row">
				<div class="col-md-6 col-sm-6 col-sm-6 col-xs-12 mt-10">
					<label class="control-label"><?php echo $label_language_values['preferred_email']." ".$label_language_values['address']; ?></label>
			...
			<div class="row">
				<div class="col-md-12 col-sm-12 col-sm-12 col-xs-12 mt-10">
