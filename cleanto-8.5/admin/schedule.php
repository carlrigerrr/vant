    ...
    for ($j = 1;$j <= 7;$j++)
    {
        ...
        <li class="active">                                                                
            ...
            { ?>                                                                    
                ...
                { ?>                                                                        <li>                                                                            <select class="selectpicker selectpickerstart" id="start_break_<?php if(isset($rrr['id'])){ echo $rrr['id']; }else{ } ?>_<?php if(isset($rrr['week_id'])){ echo $rrr['week_id']; }else{ } ?>_<?php echo $rrr['weekday_id']; ?>" data-id="<?php if(isset($rrr['id'])){ echo $rrr['id']; }else{ } ?>" data-weekid="<?php if(isset($rrr['week_id'])){ echo $rrr['week_id']; }else{ } ?>" data-weekday="<?php echo $rrr['weekday_id']; ?>" data-size="10" style="">                                                                                
        <?php $min = 0;
            while ($min < 1440)
            {
                if ($min == 1440)
            ...
            <span class="ct-staff-hours-to"> <?php echo $label_language_values['to']; ?> </span>                                                                            
            <select class="selectpicker selectpickerend" data-id="<?php if(isset($rrr['id'])){ echo $rrr['id']; }else{ } ?>" data-weekid="<?php if(isset($rrr['week_id'])){ echo $rrr['week_id']; }else{ } ?>" data-weekday="<?php echo $rrr['weekday_id']; ?>" data-size="10" style="display: none;">                                                                                <?php $min = 0;
            while ($min < 1440)
            {
                if ($min == 1440)
                {
            ...
            <button id="ct-delete-staff-break<?php if(isset($rrr['id'])){ echo $rrr['id']; }else{ } ?>_<?php echo $i; ?>_<?php echo $j; ?>" data-wiwdibi='<?php if(isset($rrr['id'])){ echo $rrr['id']; }else{ } ?>_<?php echo $i; ?>_<?php echo $j; ?>' data-break_id="<?php if(isset($rrr['id'])){ echo $rrr['id']; }else{ } ?>" class="pull-right btn btn-circle btn-default delete_break" rel="popover" data-placement='left' title="<?php echo $label_language_values['are_you_sure']; ?>?"> <i class="fa fa-trash"></i></button>                                                                            
            <div id="popover-delete-breaks<?php if(isset($rrr['id'])){ echo $rrr['id']; }else{ } ?>_<?php echo $i; ?>_<?php echo $j; ?>" style="display: none;">                                                                                
			<div class="arrow"></div>                                                                                
			<table class="form-horizontal" cellspacing="0">  
             <tbody>    
                <tr>     
                    <td>             
                     <button id="" value="Delete" data-break_id='<?php if(isset($rrr['id'])){ echo $rrr['id']; }else{ } ?>' class="btn btn-danger mybtndelete_breaks" type="submit"><?php echo $label_language_values['yes']; ?></button>                                                                                            
                     <button id="ct-close-popover-delete-breaks" class="btn btn-default close_popup" href="javascript:void(0)" data-break_id='<?php if(isset($rrr['id'])){ echo $rrr['id']; }else{ } ?>_<?php echo $i; ?>_<?php echo $j; ?>'><?php echo $label_language_values['cancel']; ?></button>  
                    </td>     
                </tr>   
            </tbody>                                                                                
			</table>                                                                            
                    ...
                    <tbody class="mytbodyfor_offtimes">                            
                        <?php $staff_id = $_SESSION['ct_adminid'];
                            ...
                            while ($r = mysqli_fetch_array($res))
                            {
                                ...
                                $edates = $ett[0];
                                $etime = $ett[1]; ?>                                            
                     <tr id="myofftime_<?php if(isset($r['id'])){ echo $r['id']; }else{ } ?>">
                     <td><?php echo $i++; ?></td>
                     ...
                     <?php  } ?>                                                
                     <td><a data-id="<?php if(isset($r['id'])){ echo $r['id']; }else{ } ?>" class='btn btn-danger ct_delete_provider left-margin'><span class='glyphicon glyphicon-remove'></span></a></td>                                            
                </tr>                                        
            <?php } } ?>                                        
        </tbody>                                    
    </table>                                
...
<div class="panel panel-default">                        
  ...
  { /* 4 */
    ...
    echo '<table align="center" class="table table-bordered table-striped monthtable">'; ?>      
          ...
          <div class="pull-left">                                    
            <div class="ct-custom-checkbox">                                        
              <ul class="ct-checkbox-list">                                            
                <li>                                                
                  <input style="margin:0px;" type="checkbox"  class="fullmonthoff all" data-prov_id="<?php echo $_SESSION['ct_adminid']; ?>" id="<?php echo $year . '-' . $this_month; ?>" <?php $offday->off_year_month = $year . '-' . $this_month;
      ...
      <?php echo '<tr><td><b>' . $label_language_values['mon'] . '</b></td><td><b>' . $label_language_values['tue'] . '</b></td>';
